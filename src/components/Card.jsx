...
export default function Card({ name, img, id, price, type }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.05, boxShadow: "0px 8px 20px rgba(255, 165, 0, 0.3)" }}
        ...
        <CardFooter className="text-medium flex-col pt-0 space-y-3">
          ...
          <div className="flex flex-wrap gap-x-4 justify-between w-full">
            ...
            <b className="text-orange-500 flex items-center gap-x-1.5">
              {type === "veg" ? <TbLeaf className="-mb-1" /> : <GiChickenOven className="-mb-0.5" />}
              {type}
            </b>
          </div>
          <Button onPress={()=>dispatch(AddItem({id:id , name:name , price:price , img:img , qty:1}))} className="bg-orange-500 w-full text-lime-100 font-semibold mt-1">
            Add to dish
          </Button>
        </CardFooter>
      </MainCard>
